  ...
  this.define = function() {
    ...
    namespace('publish', function() {
      task('fetchTags', { async: true }, function() {
        ...
        var cmds = [self.gitCmd + ' fetch --tags'];
      ...
      task('getCurrentBranch', { async: true }, function() {
        ...
        exec(self.gitCmd + ' symbolic-ref --short HEAD', function(err, stdout, stderr) {
          if (err) {
            fail(err);
          }
          if (stderr) {
      ...
      task('ensureClean', { async: true }, function() {
        ...
        exec(self.gitCmd + ' status --porcelain --untracked-files=no', function(
          err,
          stdout,
          stderr
        ) {
      ...
      task('pushVersion', ['ensureClean', 'updateVersionFiles'], { async: true }, function() {
