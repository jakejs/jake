...
var fs = require('fs');
   ...
   var taskNs = require('./task');
   var Task = taskNs.Task;
   var FileTask = taskNs.FileTask;
   var DirectoryTask = taskNs.DirectoryTask;
...
utils.mixin(jake, new (function () {
  ...
  this.parseAllTasks = function () {
    var _parseNs = function (name, ns) {
      var nsTasks = ns.tasks;
         var task;
         var nsNamespaces = ns.childNamespaces;
         var fullName;
  ...
  this.showAllTaskDescriptions = function (f) {
    var p;
       ...
       var name;
       var descr;
       var filter = typeof f == 'string' ? f : null;
  ...
  this.createPlaceholderFileTask = function (name, namespace) {
    var nsPath = '';
       var filePath = name.split(':').pop(); // Strip any namespace
       var parts;
       var fileTaskName;
       var task;
       var stats;
...
jake.setSeriesAutoPrefix = function (prefix) {
  jake._seriesAutoPrefix = prefix;
};

jake.setTaskTimeout = function (t) {
  jake._taskTimeout = t;
};

jake.series = function (...args) {
  let prereqs = args.map((arg) => {
    let name = (jake._seriesAutoPrefix || '') + arg.name;
    jake.task(name, arg);
    return name;
  });
  let seriesName = utils.string.uuid();
  ...
  Object.defineProperty(res, 'name', {value: utils.string.uuid(),
        writable: false});
  return res;
};
