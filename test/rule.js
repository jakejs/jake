...
suite('rule', function () {
  ...
  test('rule w/o pattern', function () {
    let out = exec( '../bin/cli.js -q -f jakefile.rule.js tmp').toString().trim();
    let output = [
      "tmp_dep2.c task"
      , "tmp_dep1.c task"
  ...
  test('rule w pattern w/o folder w/o namespace', function () {
    let out = exec( '../bin/cli.js  -q -f jakefile.rule.js tmp_p').toString().trim();
    let output = [
      "tmp_dep2.c task"
      , "tmp_dep1.c task"
  ...
  test('rule w pattern w folder w/o namespace', function () {
    let out = exec( '../bin/cli.js  -q -f jakefile.rule.js tmp_pf').toString().trim();
    let output = [
  ...
  test('rule w pattern w folder w namespace', function () {
    let out = exec( '../bin/cli.js -q  -f jakefile.rule.js tmp_ns').toString().trim();
    let output = [
  ...
  */
  ...
  test('rule w chain w pattern w folder w namespace', function () {
    let out = exec( '../bin/cli.js -q  -f jakefile.rule.js tmp_cr').toString().trim();
    let output = [
      "chainrule init task"
  ...
  ['precedence', 'regexPattern', 'sourceFunction'].forEach(function (key) {
    ...
    test('rule with source file not created yet (' + key  + ')', function () {
      ...
      try {
        exec('../bin/cli.js -f jakefile.rule.js ' + key + ':test');
      }
      catch(err) {
    ...
    test('rule with source file now created (' + key  + ')', function () {
      ...
      let out = exec('../bin/cli.js -q -f jakefile.rule.js ' + key + ':test').toString().trim();
