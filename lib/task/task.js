...
TaskBase = new (function () {
  ...
  this.init = function (name, prereqs, action, options) {
    ...
    if (typeof opts == 'boolean' && opts === true) {
      ...
      if (opts.concurrentLimit) {
        this.concurrentLimit = opts.concurrentLimit;
      }
    }
  };
  ...
  this.runPrereqs = function () {
    if (this.prereqs && this.prereqs.length) {
      if(this.concurrentLimit > 1) {
        ...
        async.eachLimit(currenttask.prereqs,currenttask.concurrentLimit,function(name, cb) {
          ...
          if (prereq.taskstatus === 'DONE') {
            ...
            if(prereq.taskstatus === 'UNSTARTED') {
              prereq.taskstatus = 'RUNNING';
  ...
  this.isNeeded = function () {
    if (this.taskstatus === 'DONE' || typeof this.action != 'function') {
      return false;
    }
    return true;
  };
