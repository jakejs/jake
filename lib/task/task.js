...
TaskBase = new (function () {
  ...
  this.nextPrereq = function () {
    ...
    if (name) {
      ...
      // Task doesn't exist
      // Check for existing static file or rule
      if (!prereq) {
        // Check existing static file, may be namespaced
        filePath = name.split(':').pop();
        filePath = utils.file.absolutize(filePath);

        // Does a file with that name exist?
        if (existsSync(filePath)) {
          // If the file actually exists, create a dummy FileTask
          // for it for modTime comparisons
          prereq = jake.Task[filePath];
          if (!prereq) {
            stats = fs.statSync(filePath);
            prereq = new jake.FileTask(filePath); // Abs filepath here
        ...
        // No file exists
          ...
          // Is there a Rule that can be used to synthesize
          // a FileTask for it?
          prereqRule = rule.match(parsed.name);
          if (prereqRule) {
            prereq = prereqRule.createTask(parsed.name, parsed.args);
          }
          // Neither rule nor static file, not valid
          else {
