...
TaskBase = new (function () {
  ...
  this.nextPrereq = function () {
    ...
    if (name) {
      ...
      // Task doesn't exist, may be a static file or rule
      if (!prereq) {
        // Check Rule
        prereqRule = this.namespace.matchRule(parsed.name);
        if (prereqRule) {
          prereq = prereqRule.createTask(parsed.name, parsed.args);
        }
        // Check existing static file
        else {
          prereq = jake.createPlaceholderFileTask(name, this.namespace);
          if (!prereq) {
