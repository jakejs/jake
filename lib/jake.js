...
var env = process.env
  , ENV = {};
for (var p in env) { ENV[p] = env[p]; }

var task = function (name, deps, handler, async) {
  var args = Array.prototype.slice.call(arguments)
    , type;
  args.unshift('task');
...
var file = function (name, deps, handler, async) {
  var args = Array.prototype.slice.call(arguments);
  args.unshift('file');
  jake.taskOrFile.apply(global, args);
};
...
for (var p in envVars) { ENV[p] = envVars[p]; }
...
if (opts.tasks) {
  ...
  jake.args = cmds;
  jake.runTask(taskName, taskArgs, true);
}
