...
TaskBase = new (function () {
  ...
  this.init = function (name, prereqs, action, options) {
    ...
    if (typeof opts == 'boolean' && opts === true) {
      ...
      if (opts.runparallel) {
        this.runparallel = opts.runparallel;
      }
    }
  };
  ...
  this.runPrereqs = function () {
    if (this.prereqs && this.prereqs.length) {
      if(this.runparallel > 1) {
        console.log("Run prereqs for '"+this.name+"' in parallel", this.prereqs);
        var currenttask = this;
        async.eachLimit(currenttask.prereqs,currenttask.runparallel,function(name, cb) {

          parsed = parsePrereqName(name);

          prereq = currenttask.namespace.resolveTask(parsed.name) ||
          ...
          console.log("All prereqs for task '"+currenttask.name+"' done.");
          currenttask.run();
        });
      } else {
        this.nextPrereq();
  ...
  this.complete = function (val) {
    //console.log("chain=", jake._invocationChain.map(function(c) {return c.name}));
    //console.log("will remove:", this.name);
