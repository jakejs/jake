...
utils.mixin(jake, new (function () {
  ...
  this.createTask = function () {
    ...
    if (jake.verboseTracking && name != '__root__') {
      function startHandler() {
        console.log();
        console.log("Executing " + name);
        jake.verboseTracking.startTimes[name] = new Date();
      }
      task.on('start', startHandler);
      task.on('skip', startHandler);
      task.on('complete', function() {
        jake.verboseTracking.stopTimes[name] = new Date();
        jake.verboseTracking.executionOrder.push(name);
      });      
    }    
