...
usage = ''
    ...
    + '          --verbose                  Trace extra information, including execution time.\n'
    + '';
...
Program.prototype = new (function () {
  ...
  this.init = function (configuration) {
    ...
    if (this.opts.verbose === true) {
      jake.verboseTracking = {};
      jake.verboseTracking.startTimes = {};
      jake.verboseTracking.stopTimes = {};
      jake.verboseTracking.executionOrder = [];
    }
  };
  ...
  this.run = function () {
    ...
    if (jake.verboseTracking) {
      jake.on('complete', function() {
        var executionTimes = [];

        jake.verboseTracking.executionOrder.forEach(function(task) {

          var startTime = jake.verboseTracking.startTimes[task];
          var endTime = jake.verboseTracking.stopTimes[task];

          if (typeof startTime == 'object' && typeof endTime == 'object') {
            executionTimes.push([task, startTime, endTime - startTime]);
          }
        });
        ...
        if (executionTimes.length > 0) {
          ...
          executionTimes.forEach(function(row) {
            console.log("  " + row[0] + " " + row[2] + "ms");
          });
        }
      });
    }
