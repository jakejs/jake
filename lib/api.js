...
var api = new (function () {
  ...
  /**
    ...
    @static
    @function
    @description Creates a Jake Implicit Rule
    `
    @param {String} obj_suffix The suffix name of the objective
    @param {String} src_suffix The suffix name of the objective
    @param {Array} [prereqs] Prerequisites to be run before this task
    @param {Function} [action] The action to perform for this task
    @param {Object} [opts]
      @param {Boolean} [opts.asyc=false] Perform this task asynchronously.
    ...
    rule('.o', '.c', function (obj, src) {
      var cmd = util.format('gcc -o %s %s', obj, src);
    ...
    rule('.o', '.c', ['util.h'], function (obj, src) {
      var cmd = util.format('gcc -o %s %s', obj, src);
      jake.exec([cmd], function () {
        complete();
      }, {printStdout: true});
    ...
    rule('obj/%.o', 'src/%.c', function (obj, src) {
      var cmd = util.format('gcc -o %s %s', obj, src);
      jake.exec([cmd], function () {
        complete();
      }, {printStdout: true});
    ...
    rule('%.pdf', '%.dvi', function (obj, src) {
      var cmd = util.format('dvipdfm %s',src);
      jake.exec([cmd], function () {
        complete();
      }, {printStdout: true});
    }, { async : true });

    rule('%.dvi', '%.tex', function (obj, src) {
      var cmd = util.format('latex %s',src);
      jake.exec([cmd], function () {
        complete();
      }, {printStdout: true});
    }, { async : true });
   ...
   */
  this.rule = function (obj_suffix, src_suffix, prereqs, action, opts) {
    var args = Array.prototype.slice.call(arguments);
    jake.currentNamespace.rules[ obj_suffix ] = {
        'rule' : args
