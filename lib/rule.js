...
Matcher = new (function () {
  ...
  this.source = function(prereq, objP, srcP) {
    ...
    if (objP.indexOf('%') == -1) {
      // A Simple Suffix
      src = this.stringReplaceSuffix(prereq, objP, srcP);
      return {
        dep: src
      , file: src
      };
...
rule = new (function () {
  ...
  this.createTask = function(pattern, prereq, args) {
    ...
    task = Matcher.split(prereq);
    name = task.name;
    ns = task.ns;

    // Obtain the matching rule.
    args = ns.rules[pattern].args.slice(0);  // We need a copy.
    ...
    src = Matcher.source( prereq, objP, srcP );
    ...
    if (Array.isArray(rule[0])) {
      prereqs = args.shift().slice(0); // again, a copy
    }
    prereqs.unshift( src.dep );

    // Fetch the action and opts.
    ...
    // Now, create the file task.
    newArgs = [name, prereqs, action, opts];
    ...
    createdTask = jake.createTask.apply(global, newArgs);
