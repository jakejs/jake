...
jake = new function () {
  ...
  this.createTree = function (name, includePrereqs) {
          ...
          if (task.type == 'file') {
            ...
            if (prereqs.length) {
              for (var i = 0, ii = prereqs.length; i < ii; i++) {
                ...
                if (this.getTask(prereq).type == 'task' || !modTime || _modTimes[prereq] > modTime) {
                  if (typeof task.action == 'function') {
                    task.action.apply(task, args || []);
                    // The action may have modified the file
                    stats = fs.statSync(name);
                    modTime = stats.ctime;
                  }
                  break;
