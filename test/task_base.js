...
, 'test default task': function (next) {
    h.exec('node ../bin/cli.js', function (out) {
      assert.equal('default task', out);
      h.exec('node ../bin/cli.js default', function (out) {
        assert.equal('default task', out);
        next();
      });
    });
...
, 'test task with no action': function (next) {
    h.exec('node ../bin/cli.js noAction', function (out) {
      assert.equal('default task', out);
      next();
    });
  }
...
, 'test a task with no action and no prereqs': function (next) {
    h.exec('node ../bin/cli.js noActionNoPrereqs', function () {
      next();
    });
  }

, 'test passing args to a task': function (next) {
    h.exec('node ../bin/cli.js argsEnvVars[foo,bar]', function (out) {
...
, 'test a task with environment vars': function (next) {
    h.exec('node ../bin/cli.js argsEnvVars foo=bar baz=qux', function (out) {
      var parsed = h.parse(out)
        , env = parsed.env;
...
, 'test passing args and using environment vars': function (next) {
    h.exec('node ../bin/cli.js argsEnvVars[foo,bar] foo=bar baz=qux', function (out) {
      var parsed = h.parse(out)
        , args = parsed.args
        , env = parsed.env;
      assert.equal(args[0], 'foo');
...
, 'test a simple prereq': function (next) {
    h.exec('node ../bin/cli.js foo:baz', function (out) {
...
, 'test a duplicate prereq only runs once': function (next) {
    h.exec('node ../bin/cli.js foo:asdf', function (out) {
      assert.equal('foo:bar task\nfoo:baz task\nfoo:asdf task', out);
      next();
    });
  }
...
, 'test a prereq with command-line args': function (next) {
    h.exec('node ../bin/cli.js foo:qux', function (out) {
...
, 'test a prereq with args via invoke': function (next) {
    h.exec('node ../bin/cli.js foo:frang[zxcv,uiop]', function (out) {
...
, 'test prereq execution-order': function (next) {
    h.exec('node ../bin/cli.js hoge:fuga', function (out) {
...
, 'test basic async task': function (next) {
    h.exec('node ../bin/cli.js bar:bar', function (out) {
...
, 'test modifying a task by adding prereq during execution': function (next) {
    h.exec('node ../bin/cli.js voom', function (out) {
      assert.equal(2, out);
      next();
    });
  }
...
, 'test listening for task error-event': function (next) {
    h.exec('node ../bin/cli.js vronk:groo', function (out) {
