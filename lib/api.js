...
var api = new (function () {
  ...
  /**
    ...
    arguments completes the last task on the invocationChain. If you use concurrent
    execution of prereqs this will probably complete a wrong task. You can call this
    function with this task as the first argument, before the optional return value.
    Alternatively you can call task.complete() (this is preferred)
    `
    @example
    task('generate', ['doc:clobber'], function () {
  ...
  this.complete = function (task, val) {
    ...
    if(task && task. _currentPrereqIndex >=0 ) {
      ...
      if(jake._invocationChain.length > 0) {
        ...
        console.log("Complete on empty chain");
      }
    }
  };
